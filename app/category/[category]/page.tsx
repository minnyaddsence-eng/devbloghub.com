...
export default async function CategoryPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-5xl min-w-0 px-3 py-8 sm:px-4 sm:py-12">
      <p className="text-sm font-medium text-sky-700 dark:text-cyan-300/80">Category</p>
      <h1 className="mt-2 break-words text-2xl font-bold text-slate-900 sm:text-3xl md:text-4xl dark:text-white">{name} tools</h1>
      ...
      <div className="mt-8 grid grid-cols-1 gap-3 sm:mt-10 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
        {tools.map((t) => (
          <Link key={t.slug} href={`/tools/${t.slug}`} className="min-w-0">
            <GlassPanel className="h-full p-4 transition hover:border-sky-400/50 hover:bg-sky-50/40 dark:hover:border-cyan-400/40 dark:hover:bg-white/[0.09] sm:p-5">
              <h2 className="break-words text-base font-semibold text-slate-900 sm:text-lg dark:text-white">{t.name}</h2>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{t.description}</p>
              <p className="mt-3 text-xs uppercase tracking-wide text-sky-700 dark:text-cyan-300/70">Open tool →</p>
            </GlassPanel>
          </Link>
        ))}
      </div>
